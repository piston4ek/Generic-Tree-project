...
{
  // First we have to be sure that our tree is empty for new data.
  // We're going to use clear() method that extra including
  // check is that tree is empty already
  tree.clear();
  
  // And then add childrens to tree
  // by using queue
  std::queue<GenericTree<int>::TreeNode*> nodeToAdd;
  
  // Create a root and add to queue
  nodeToAdd.push(tree.createRoot(4));
  // Add childrens to root and add in queue 8
  nodeToAdd.push(nodeToAdd.front()->addChild(8));
  nodeToAdd.front()->addChild(15);
  // Pop 4 from queue
  nodeToAdd.pop();
  // Add 16 to queue
  nodeToAdd.push(nodeToAdd.front()->addChild(16));
  // Create 23
  nodeToAdd.front()->addChild(23);
  // Pop 8
  nodeToAdd.pop();
  // Add 42
  nodeToAdd.front()->addChild(42);
  nodeToAdd.pop();
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // We will use queue of Nodes for exploring 
  ...
  while(!nodesToExplore.empty())
  {
    TreeNode* temp = nodesToExplore.front();
    // Add data to result-vector
    results.push_back(temp->data);
    
    // And explore childrens and add to queue
    for(auto childPtr : temp->childrenPtrs)
    {
      // Nullptr case: we shouldn't append data
      if(!childPtr){
        continue;
      }
    ...
    // And pop from queue current node
    nodesToExplore.pop();
  }
